...
async function runTest() {
  ...
  assert(
    result1.success,
    "Public channel name message sending should be successful",
  );
  assert(result2, "Public channel ID response should not be null");
  assert(
    result2.success,
    "Public channel ID message sending should be successful",
  );
  assert(result3, "Private channel name response should not be null");
  assert(
    result3.success,
    "Private channel name message sending should be successful",
  );
  assert(result4, "Private channel ID response should not be null");
  assert(
    result4.success,
    "Private channel ID message sending should be successful",
  );

  console.log(
    "Send Message Test Response 1 (public name): " +
      JSON.stringify(result1, null, 2),
    "Send Message Test Response 2 (public ID): " +
      JSON.stringify(result2, null, 2),
    "Send Message Test Response 3 (private name): " +
      JSON.stringify(result3, null, 2),
    "Send Message Test Response 4 (private ID): " +
